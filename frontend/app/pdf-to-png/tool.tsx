  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      for (let i = 1; i <= numPages; i++) {
        setStatusText(`Rendering page ${i} of ${numPages} to PNG...`);
      ...
      console.error(err);
      alert(err instanceof Error ? err.message : "Failed to convert PDF to PNG images.");
    } finally {
      setProcessing(false);
    }
  };
    ...
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
        ...
        {!file ? (
          <div className="rounded-md border-[3px] border-dashed border-[#9da0d9] bg-white p-8 text-center hover:border-[#4449A6]">
            ...
            <p className="mb-3 text-base font-medium text-[#6e6e6e]">
              Select a PDF file to convert to PNG images
            </p>
            <label
              htmlFor="pdf-png-upload"
      ...
      <div className="sticky top-20 flex h-fit flex-col gap-4 rounded-md border border-[#d9dcea] bg-white p-5 lg:col-span-4">
        ...
        <p className="text-xs leading-relaxed text-[#6e7288]">
          Extract pages from your PDF as crystal clear lossless PNG images with transparent background support.
        </p>
