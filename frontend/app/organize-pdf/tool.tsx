...
import { reorderPdfPages, createPdfBlob } from "@/lib/pdf-tools";
import { formatBytes } from "@/lib/config";

export function OrganizePdfTool() {
  const [file, setFile] = useState<File | null>(null);
  const [orderStr, setOrderStr] = useState("1, 2, 3");
  ...
  const handleReorder = async () => {
    if (!file || !orderStr.trim()) return;
    setProcessing(true);
    try {
      const pageNumbers = orderStr
        .split(",")
        .map((s) => parseInt(s.trim(), 10))
        .filter((n) => !isNaN(n));
      const organizedBytes = await reorderPdfPages(file, pageNumbers);
      setResultBlob(createPdfBlob(organizedBytes));
    } catch (err) {
      console.error(err);
      alert(err instanceof Error ? err.message : "Failed to reorder PDF pages.");
    } finally {
      setProcessing(false);
    }
  };
        ...
        {!file ? (
          <div className="rounded-md border-[3px] border-dashed border-[#9da0d9] bg-white p-8 text-center hover:border-[#4449A6]">
            ...
            <p className="mb-3 text-base font-medium text-[#6e6e6e]">
              Select a PDF file to reorder and organize pages
            </p>
            <label
              htmlFor="pdf-org-upload"
      ...
      <div className="sticky top-20 flex h-fit flex-col gap-4 rounded-md border border-[#d9dcea] bg-white p-5 lg:col-span-4">
        ...
        <div>
          ...
          <p className="mt-1 text-[11px] text-gray-500">
            Specify the sequence of page numbers separated by commas (e.g. <b>2, 1, 3</b> swaps page 1 and 2).
          </p>
        </div>
        ...
        >
          ...
          {processing ? "Reordering..." : "Reorder PDF Pages"}
        </button>
      </div>
    </div>
  );
