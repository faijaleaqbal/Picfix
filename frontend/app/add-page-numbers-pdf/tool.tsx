...
import { addPageNumbersToPdf, createPdfBlob } from "@/lib/pdf-tools";
...
export function AddPageNumbersTool() {
  ...
  const handleAddNumbers = async () => {
    ...
    setProcessing(true);
    try {
      const numberedBytes = await addPageNumbersToPdf(file, position);
      setResultBlob(createPdfBlob(numberedBytes));
    } catch (err) {
      console.error(err);
      alert(err instanceof Error ? err.message : "Failed to add page numbers to PDF.");
    } finally {
      setProcessing(false);
    }
  };
      ...
      <div className="sticky top-20 flex h-fit flex-col gap-4 rounded-md border border-[#d9dcea] bg-white p-5 lg:col-span-4">
        ...
        <div className="space-y-2">
          <label className="flex cursor-pointer items-center gap-2 rounded border border-gray-200 p-2.5 text-xs font-semibold text-gray-700 hover:bg-gray-50">
            ...
            />
            <span>Bottom Center (Recommended)</span>
          </label>
          <label className="flex cursor-pointer items-center gap-2 rounded border border-gray-200 p-2.5 text-xs font-semibold text-gray-700 hover:bg-gray-50">
            <input
              type="radio"
